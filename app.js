...
// Grab all product elements from NEW COLLECTION
const productElements = document.querySelectorAll(".cards .product");
...
function performSearch() {
  ...
  productElements.forEach((product) => {
    const name = product.querySelector(".pname").textContent.toLowerCase();
    const meta = product.querySelector(".pmeta").textContent.toLowerCase();
    const price = product.querySelector(".pprice").textContent.toLowerCase();

    if (name.includes(query) || meta.includes(query) || price.includes(query)) {
      const div = document.createElement("div");
      div.classList.add("result-item");
      div.innerHTML = `
          <strong>${product.querySelector(".pname").textContent}</strong> — ${
        product.querySelector(".pprice").textContent
      }
        `;
      div.onclick = () => {
        searchInput.value = product.querySelector(".pname").textContent;
        resultsContainer.innerHTML = "";
      };
